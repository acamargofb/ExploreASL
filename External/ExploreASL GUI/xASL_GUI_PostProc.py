...
class DandD_ListWidget2LineEdit(QLineEdit):
    """
    Small, modified QLineEdit to support accepting text drops from a QListWidget or QAbstractItemModel derivatives
    """

    def __init__(self, data_dtypes, permitted_dtypes, parent=None):
        super().__init__(parent)
        self.data_dtypes = data_dtypes
        self.permitted_dtypes = permitted_dtypes
    ...
    def dropEvent(self, event: QDropEvent) -> None:
        if all([event.mimeData().hasFormat("application/x-qabstractitemmodeldatalist"),
            ...
            colname = ix.data()
            # print(f"For column: {colname}, the detected datatype was: {self.data_dtypes[colname]}")
            # print(f"The permitted dtypes for this widget are: {self.permitted_dtypes}")
            if self.data_dtypes[colname] in self.permitted_dtypes:
                event.accept()
